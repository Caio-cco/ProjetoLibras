    ...
    const fetchData = async () => {
      try {
        ...
        const frasesCompletas = data.frases.map((frase) => {
          const palavrasDaFrase = data.palavras
          ...
          return {
            ...
            palavrasDisponiveis: palavrasDaFrase.map(p => ({
              label: p.texto_img,
              caminho: `http://localhost:5010/palavrasEFrases/${p.url_img}`,
            })),
            respostaCorreta: palavrasDaFrase.map(
              p => `http://localhost:5010/palavrasEFrases/${p.url_img}`
            ),
          };
        });

        console.log("Frases montadas:", frasesCompletas);
        setFrasesQuiz(frasesCompletas);
        setLoading(false);
      } catch (err) {
        console.error(err);
