            ...
            <>
              ...
              <div className="modal-actions">
                ...
                {acertos < Math.ceil(total * 0.6) && (
                  <button className="btn-refazer" onClick={onRefazer}>Refazer Atividade</button>
                )}
              </div>
            </>
          )}
    ...
    <div className="associacao-basico">
      ...
      <div className="conteudo-principal">
        <div className="card-atividade">
          <div className="progress-container">
            <span className="pergunta-numero">Frase {indiceAtual + 1} de {totalPerguntas}</span>
            <div className="barra-progresso">
              <div className="preenchimento" style={{ width: `${totalPerguntas ? ((indiceAtual + 1) / totalPerguntas) * 100 : 0}%` }}></div>
            </div>
          </div>
          <section className="secao-frase-alvo-visual">
            <h2 className="titulo-frase-alvo">"{perguntaAtual ? perguntaAtual.frase : "Carregando..."}"</h2>
            <p className="instrucao-quiz">Monte a frase em LIBRAS, clicando nos sinais disponíveis abaixo.</p>
          </section>
          <section className="secao-resposta">
            <div className="area-montagem-frase">
              <h3 className="titulo-resposta">Sua resposta em Sinais:</h3>
              <FraseMontadaDisplay fraseMontada={fraseMontada} onRemover={!statusVerificacao ? removerUltimaPalavra : undefined} />
            </div>
            ...
            <div className="botoes-acao">
              ...
              {statusVerificacao === "incorreta" && (
                <button className="btn-tentar-novamente" onClick={() => { setFraseMontada([]); setStatusVerificacao(null); }}>Tentar Novamente</button>
              )}
            </div>
          </section>
        </div>
      ...
      {modal && (
        <FeedbackModal
          ...
          onRefazer={reiniciarJogo}
          onVoltarAtividades={() => {
            if (modal.tipo === "fim") navigate("/atividades");
            else handleProximaFrase();
          }}
        />
      )}
      <Rodape />
